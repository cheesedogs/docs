...
| 前置条件 | 1、用户身份为管理员<br/>2、最新市场调研结果已出 |  |  |
| 后置条件 | 1、系统提示管理员添加成功<br/>2、系统中已上架影片列表添加管理员确定的上架影片 |  |  |
| 优先级 | 1000 |  |  |
| 正常流程 | 1、管理员发起上架电影请求 |  |  |
|  | 2、系统显示上架电影界面 |  |  |
|  | 3、管理员输入上架影片的详情，包括：影片名、影片ID、影片类型、电影宣传海报、票价、剧情介绍、演职人员、制片国家/地区、语言、上映日期、片长和下映日期 |  |  |
|  | 4、管理员确认并提交刚才输入的影片信息                        |  |  |
|  | 5、系统记录并显示已上架的影片清单，电影清单由多个影片预览图组成，影片预览图结构为影片海报、影片名、上映状态、想看人数、剧情介绍和演职人员 |  |  |
|  | 6、系统提示管理员添加成功并跳转到管理员主界面 |  |  |
|  | 7、管理员重复1~6步，直到完成所有影片的上架                   |  |  |
| 扩展流程 | 3a、管理员取消上架该影片 |  |  |
|  | 4a、影片库中没有需要上架的影片： |  |  |
|  | 2a、系统提示没有该影片，并拒绝输入 |  |  |
|  | 3、返回正常流程第2步 |  |  |
|  | 4b、影片必填信息没有全部填写 |  |  |
|  | 2b、系统提示影片信息不足，并拒绝输入 |  |  |
| | 4c、管理员输入的上映日期在系统时间之前 | | |
| | 2c、系统提示上映日期输入不能在当前系统日期之前，并拒绝输入 | | |
| | 4d、管理员输入的下映日期在上映日期之前 | | |
| | 2d、系统提示下映日期输入不能在上映日期之前，并拒绝输入 | | |
| 特殊需求 | 无 |  |  |
...
| 触发条件 | 影院拥有新的可供放映的影厅                                   |                |           |
| 前置条件 | 1、用户身份是管理员<br/>2、管理员确认新的影厅可供放映        |                |           |
| 后置条件 | 1、系统提示用户影厅信息录入成功<br/>2、影厅数据库中增加管理员新录入影厅的记录 |                |           |
| 优先级   |                                                              |                |           |
| 正常流程 | 1、管理员发起录入影厅信息的请求                              |                |           |
|          | 2、系统显示录入影厅信息的界面                                |                |           |
|          | 3、管理员输入影厅信息，包括名称、座位信息、大/中/小等        |                |           |
|          | 4、管理员确认并提交要录入的影厅信息                          |                |           |
|          | 5、系统提示管理员录入成功并跳转到管理员主界面                |                |           |
...
| 触发条件 | 影院某一影厅的信息需要修改                                   |                |           |
| 前置条件 | 1、用户身份是管理员<br/>2、管理员确认某一影厅信息需要修改    |                |           |
| 后置条件 | 1、系统提示用户影厅信息修改成功<br/>2、影厅数据库中对应影厅的记录被修改 |                |           |
| 优先级   |                                                              |                |           |
| 正常流程 | 1、管理员发起修改影厅信息的请求                              |                |           |
|          | 2、系统显示修改影厅信息的界面                                |                |           |
|          | 3、管理员更改对应影厅的信息，如名称、座位信息、大/中/小等    |                |           |
|          | 4、管理员确认并提交要修改的影厅信息                          |                |           |
|          | 5、系统提示管理员修改成功并跳转到管理员主界面                |                |           |
|          | 6、管理员重复1~5步，直到完成所有影厅信息的修改               |                |           |
| 扩展流程 | 3a、管理员取消修改该影厅的信息                               |                |           |
|          | 4a、影厅必填信息没有全部填写                                 |                |           |
|          | 2a、系统提示影厅信息不足，并拒绝输入                         |                |           |
| 特殊需求 | 无                                                           |                |           |

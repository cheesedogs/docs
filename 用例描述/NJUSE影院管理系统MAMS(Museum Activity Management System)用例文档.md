...
| -------- | ----------------------------------------------------------- | ---------------- | ---------- |
| 创建者   | 周际宇                                                      | 最后一次更新者   | 周际宇     |
| 创建日期 | 2019/5/12                                                   | 最后一次更新日期 | 2019/5/13  |
| 参与者   | 观众，目标是购买会员卡                                      |                  |            |
| 触发条件 | 用户需要购买会员卡                                          |                  |            |
| 前置条件 | 无                                                          |                  |            |
...
|          | 2、系统提示用户选择或输入充值金额             |                  |            |
|          | 3、用户根据提示输入选择或输入充值金额         |                  |            |
|          | 4、系统显示订单信息，包括充值金额、支付金额等 |                  |            |
|          | 5、用户确认订单并支付                         |                  |            |
|          | 6、系统提示充值成功并显示更新后的会员卡信息   |                  |            |
| 扩展流程 | 3a、用户取消充值                              |                  |            |
|          | 4、系统返回正常流程第1步                      |                  |            |
|          | 5a、用户取消订单                              |                  |            |
|          | 6、系统返回正常流程第3步                      |                  |            |
...
| -------- | ------------------------------------------------------------ | -------------- | ------------------ |
| 创建者   | 江辉                                                         | 最后一次更新者 | 江辉               |
| 创建日期 | 2019/5/12                                                    | 最后更新日期   | 2019/5/13          |
...
|          | 2、系统显示电影列表（电影列表中每个电影都要显示包括其名称、电影宣传海报、主演、上映日期等简单资料） |                |                    |
| 扩展流程 | 无                                                           |                |                    |
| 特殊需求 | 无                                                           |                |                    |
...
|          | 3、用户选择想要查看的影厅和日期                              |                |                        |
|          | 4、系统根据用户的选择展示该日期下该影厅的排片信息表（排片信息表字段定义：影厅名，影片名，放映开始时间，放映结束时间，票价） |                |                        |
| 扩展流程 | 无                                                           |                |                        |
| 特殊需求 | 选择日期时（默认当天），可以选择日期区间或具体日期，选择影厅时（默认全部），可以选择全部影厅或某具体影厅。如：选择2019-04-01到2019-04-08区间全部影厅的排片信息<br /> |                |                        |
...
|          | 3、用户选择想要查看的电影                                    |                |                        |
|          | 4、系统展示该电影在各影厅的排片信息表(排片信息表字段定义：影厅名，影片名，放映开始时间，放映结束时间，票价) |                |                        |
| 扩展流程 | 无                                                           |                |                        |
| 特殊需求 | 无                                                           |                |                        |
...
|          | 6、系统提示添加成功                                    |                |              |
| 扩展流程 | 5a、用户选择的影厅该时间段与已经排片的其他影片冲突     |                |              |
|          | 6、系统提示时间冲突，请用户重新选择，返回正常流程第5步 |                |              |
| 特殊需求 | 无                                                     |                |              |
...
|          | 3、系统提示时间段时间段不得小于影片时长，返回正常流程第1步 |                |              |
|          | 5b、用户输入的新时间段时长小于当前影片放映时长             |                |              |
|          | 6、系统提示错误，并返回正常流程第5步                       |                |              |
|          | 2a、用户选择的影片放映时长大于当前时间段                   |                |              |
|          | 3、系统提示时间段不得小于影片时长，返回正常流程第5步       |                |              |
| 特殊需求 | 无                                                         |                |              |
...
|          | 7、用户确认删除                                              |                |              |
|          | 8、系统提示删除成功                                          |                |              |
| 扩展流程 | 4a、当前时间不早于排片信息对观众可见时间                     |                |              |
|          | 5、系统提示当前时间不早于排片信息对观众可见时间，无法删除并返回正常流程第2步 |                |              |
|          | 7a、用户选择取消                                             |                |              |
|          | 8、返回正常流程第4步                                         |                |              |
| 特殊需求 | 无                                                           |                |              |
...
| -------- | ---------------------------------------------------------- | -------------- | ------------------------ |
| 创建者   | 江辉                                                       | 最后一次更新者 | 许竣博                   |
| 创建日期 | 2019/5/12                                                  | 最后更新日期   | 2019/5/4                 |
...
|          | 5、系统提示输入时间晚于排片日期，返回正常流程第2步         |                |                          |
|          | 4b、用户输入时间晚于排片的开始放映时间                     |                |                          |
|          | 5、系统提示输入时间晚于排片开始放映时间，返回正常流程第2步 |                |                          |
| 特殊需求 | 无                                                         |                |                          |
...
|          | 5、系统提示该时间范围内没有票房信息并返回正常流程2           |                |                |
|          | 4b、用户输入的查看到第几位超出近期有票房的电影数量           |                |                |
|          | 5、系统提示近期有票房的电影数量不足并显示所有近期票房数据    |                |                |
...
|          | 5、系统提示截止时间应在当天之后并返回正常流程第2步           |                |              |
|          | 4b、用户输入的优惠券使用有效期在当天之前                     |                |              |
|          | 5、系统提示截止时间应在当天之后并返回正常流程第2步           |                |              |
|          | 4c、用户输入的金额为负数                                     |                |              |
|          | 5、系统提示金额应为非负数并返回正常流程第2步                 |                |              |
...
| 后置条件 | 1、系统中已上架影片信息中添加管理员确定的上架影片 |  |  |
| 优先级 |  |  |  |
| 正常流程 | 1、用户发起上架电影请求 |  |  |
|  | 2、系统提示用户输入影片的详情，包括：影片名、影片ID、影片类型、电影宣传海报、票价、剧情介绍、演职人员、制片国家/地区、语言、上映日期、片长和下映日期 | | |
|  | 3、用户根据提示输入上架影片的详情 |  |  |
|  | 4、系统提示上架成功 |  |  |
| 扩展流程 | 3a、用户取消上架该影片 |  |  |
|  | 4、返回正常流程第1步 | | |
|  | 4a、影片库中没有需要上架的影片 |  |  |
|  | 5、系统提示该影片不存在，返回正常流程第2步 |  |  |
|  | 4b、影片必填信息没有全部填写 |  |  |
|  | 5、系统提示影片信息不足，返回正常流程第2步 |  |  |
| | 4c、管理员输入的上架日期在系统时间之前 | | |
| | 5、系统提示上映日期输入不能在当前系统日期之前，并返回正常流程第2步 | | |
| 特殊需求 | 无 |  |  |
...
| 创建者   | 刘涛                                                         | 最后一次更新者 | 刘涛         |
| 创建日期 | 2019/5/12                                                    | 最后更新日期   | 2019/5/13    |
| 参与者   | 管理员，目的是将新的影厅信息添加到系统中                     |                |              |
| 触发条件 | 管理员需要录入新的影厅信息                                   |                |              |
| 前置条件 | 1、影院拥有新的可供放映的影厅<br />2、用户的管理员身份已经被识别和授权 |                |              |
| 后置条件 | 1、影厅信息中增加新录入的影厅的信息                          |                |              |
| 优先级   |                                                              |                |              |
| 正常流程 | 1、用户发起录入影厅信息的请求                                |                |              |
|          | 2、系统提示用户输入需要录入的影厅信息                        |                |              |
...
| 触发条件 | 管理员需要对影院某一影厅的信息进行修改                       |                |              |
| 前置条件 | 1、用户的管理员身份已经被识别和授权                          |                |              |
| 后置条件 | 1、影厅信息中对应影厅的信息被修改                            |                |              |
| 优先级   |                                                              |                |              |
| 正常流程 | 1、用户发起修改影厅信息的请求                                |                |              |
|          | 2、系统提示用户选择或输入要修改的影厅                        |                |              |
|          | 3、用户选择或输入需要修改信息的影厅                          |                |              |
|          | 4、系统显示对应影厅信息并提示用户输入需要修改的信息          |                |              |
|          | 5、用户根据提示输入要修改的信息，如名称、座位信息、大/中/小等并提交 |                |              |
|          | 6、系统提示修改成功                                          |                |              |
| 扩展流程 | 3a、用户输入的影厅不存在                                     |                |              |
|          | 4、系统提示不存在对应的影厅，并返回正常流程第2步             |                |              |
|          | 4a、用户取消修改该影厅的信息                                 |                |              |
|          | 5、返回正常流程第1步                                         |                |              |
| 特殊需求 | 无                                                           |                |              |
...
| 后置条件 | 1、会员卡信息中增加新的优惠策略                              |                |                    |
| 优先级   |                                                              |                |                    |
| 正常流程 | 1、用户发起发布优惠策略的请求                                |                |                    |
|          | 2、系统提示用户输入新的优惠策略                              |                |                    |
|          | 3、用户输入新的优惠策略并提交                                |                |                    |
|          | 4、系统提示用户发布成功                                      |                |                    |
| 扩展流程 | 3a、用户取消发布该优惠策略                                   |                |                    |
...
| 前置条件 | 1、用户的管理员身份已经被识别和授权<br/>2、最新调研结果及会议讨论结果已出 |                |                    |
| 后置条件 | 1、会员卡信息中对应的优惠策略的记录被修改                    |                |                    |
| 优先级   |                                                              |                |                    |
| 正常流程 | 1、用户发起修改优惠策略的请求                                |                |                    |
|          | 2、系统提示用户选择优惠策略                                  |                |                    |
|          | 3、用户根据提示选择需要修改的优惠策略                        |                |                    |
|          | 4、系统提示用户输入优惠策略                                  |                |                    |
|          | 5、用户输入要修改的优惠策略并提交                            |                |                    |
...
|          | 4、返回正常流程第1步                                         |                |                    |
|          | 6a、用户输入的优惠策略错误                                   |                |                    |
|          | 7、系统提示输入格式有误，并返回正常流程第4步                 |                |                    |
| 特殊需求 | 1、优惠策略包括充值返利，内容包括为活动时间和返利金额<br />2、优惠策略的活动时间不得早于当前时间，结束时间不得早于开始时间，返利金额为非负数 |                |                    |
...
| 前置条件 | 1、用户的管理员身份已经被识别和授权<br/>2、最新调研结果及会议讨论结果已出 |                |              |
| 后置条件 | 1、增加系统中影票信息的退票策略                              |                |              |
| 优先级   |                                                              |                |              |
| 正常流程 | 1、用户请求设置退票策略                                      |                |              |
|          | 2、系统提示用户输入退票策略，包括能否退票、退票时间期限、价格折算等 |                |              |
|          | 3、用户根据提示输入退票策略并提交                            |                |              |
|          | 4、系统提示设置成功                                          |                |              |
| 扩展流程 | 3a、用户取消发布该退票策略                                   |                |              |
|          | 4、返回正常流程第1步                                         |                |              |
...
| 前置条件 | 1、用户的管理员身份已经被识别和授权<br/>2、最新调研结果及会议讨论结果已出 |                |              |
| 后置条件 | 1、修改系统中当前影票信息中的退票策略                        |                |              |
| 优先级   |                                                              |                |              |
| 正常流程 | 1、用户请求修改退票策略                                      |                |              |
|          | 2、系统显示当前的退票策略并提示用户输入需要修改的信息        |                |              |
|          | 3、用户根据提示输入需要修改的信息并提交                      |                |              |
|          | 4、系统提示修改成功                                          |                |              |
| 扩展流程 | 3a、用户取消修改当前的优惠策略                               |                |              |
|          | 4、返回正常流程第1步                                         |                |              |
|          | 4a、用户输入的时间期限在规定之外                             |                |              |
|          | 5、系统提示输入的时间期限有误，并返回正常流程第2步           |                |              |
| 特殊需求 | 若在修改退票策略时有观众用户在退票，系统需提示其根据最新退票策略提交退票请求 |                |              |
...
| 后置条件 | 无                                                           |                |                        |
| 优先级   |                                                              |                |                        |
| 正常流程 | 1、用户请求查看所有电影的想看人数                            |                |                        |
|          | 2、系统显示所有电影截止到当前系统时间为止的想看人数          |                |                        |
|          | 3、用户选择具体的一部电影查看                                |                |                        |
|          | 4、系统显示该电影自发布时间到当前日期为止每天想看人数的变化趋势 |                |                        |
| 扩展流程 | 无                                                           |                |                        |
| 特殊需求 | 1、系统以柱状图的形式显示所有电影截止到当前系统时间为止的想看人数<br />2、系统以折线图的形式显示该电影自发布时间到当前日期为止每天想看人数的变化趋势，即纵坐标为“想看人数”，横坐标为“日期”。 |                |                        |
...
| 创建日期 | 2019/5/12                                                    | 最后更新日期   | 2019/5/14    |
| 参与者   | 管理员，目的是增加影院角色                                   |                |              |
| 触发条件 | 管理员根据人事变动需要增加影院角色                           |                |              |
| 前置条件 | 1、用户的管理员身份已经被识别和授权<br/>2、影院人事发生变动  |                |              |
| 后置条件 | 1、系统中影院角色信息中对应的记录发生预期中的增加            |                |              |
| 优先级   |                                                              |                |              |
| 正常流程 | 1、用户请求增加影院角色                                      |                |              |
|          | 2、系统提示用户输入需要增加的角色的信息                      |                |              |
|          | 3、用户根据提示输入角色的信息(包括ID、用户名、密码、身份)并提交 |                |              |
|          | 4、系统显示增加成功                                          |                |              |
| 扩展流程 | 3a、用户取消增加角色                                         |                |              |
|          | 4、返回正常流程第1步                                         |                |              |
|          | 4a、用户输入的角色信息格式错误                               |                |              |
...
| 创建日期 | 2019/5/12                                                    | 最后更新日期   | 2019/5/14    |
| 参与者   | 管理员，目的是修改影院角色                                   |                |              |
| 触发条件 | 管理员根据人事变动需要修改影院角色                           |                |              |
| 前置条件 | 1、用户的管理员身份已经被识别和授权<br/>2、影院人事发生变动  |                |              |
| 后置条件 | 1、系统中影院角色信息中对应的记录发生预期中的修改            |                |              |
| 优先级   |                                                              |                |              |
| 正常流程 | 1、用户请求修改影院角色信息                                  |                |              |
|          | 2、系统提示用户输入需要修改的角色的信息                      |                |              |
|          | 3、用户根据提示输入角色的信息(包括ID、用户名、密码、身份)并提交 |                |              |
|          | 4、系统显示修改成功                                          |                |              |
| 扩展流程 | 3a、用户取消修改角色信息                                     |                |              |
|          | 4、返回正常流程第1步                                         |                |              |
...
| -------- | ------------------------------------------------- | ---------------- | ------------ |
| 创建者   | 周际宇                                            | 最后一次更新者   | 周际宇       |
| 创建日期 | 2019/5/14                                         | 最后一次更新日期 | 2019/5/14    |
| 参与者   | 管理员，目标是查找影院角色                        |                  |              |
| 触发条件 | 管理员需要查找影院角色                            |                  |              |
| 前置条件 | 1、用户的管理员身份已经被识别和授权               |                  |              |
| 后置条件 | 无                                                |                  |              |
...
| 创建日期 | 2019/5/12                                                   | 最后更新日期   | 2019/5/14    |
| 参与者   | 管理员，目的是删除影院角色                                  |                |              |
| 触发条件 | 管理员根据人事变动需要删除影院角色                          |                |              |
| 前置条件 | 1、用户的管理员身份已经被识别和授权<br/>2、影院人事发生变动 |                |              |
| 后置条件 | 1、系统中影院角色信息中对应的记录发生预期中的删除           |                |              |
| 优先级   |                                                             |                |              |
| 正常流程 | 1、用户请求删除影院角色                                     |                |              |
|          | 2、系统显示影院角色列表并提示用户选择需要删除的角色         |                |              |
|          | 3、用户根据提示选择需要删除并提交                           |                |              |
|          | 4、系统显示删除成功                                         |                |              |
| 扩展流程 | 3a、用户取消删除角色                                        |                |              |
|          | 4、返回正常流程第1步                                        |                |              |
